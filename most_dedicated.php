...
class GetMostRecentDetails {
    ...
    function most_sent($results, $page) {
			...
			if ($page > 0) {
				...
				$query = "SELECT *, COUNT(*) AS WSDedicationQuantity FROM mtvdedicateapp_posts WHERE WSDedicationSuccess = 1 AND WSDedicationPrivate = 0 GROUP BY WSDedicationVideoID ORDER BY WSDedicationQuantity DESC LIMIT ".$offset.", ".$results;
				
			} else {
				$query = "SELECT *, COUNT(*) AS WSDedicationQuantity FROM mtvdedicateapp_posts WHERE WSDedicationSuccess = 1 AND WSDedicationPrivate = 0 GROUP BY WSDedicationVideoID ORDER BY WSDedicationQuantity DESC LIMIT ".$results;
			}
