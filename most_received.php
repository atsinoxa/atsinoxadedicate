...
class GetMostRecentDetails {
    ...
    function most_received($results, $page) {
			...
			if ($page > 0) {
				...
				$query = "SELECT *, COUNT(*) AS WSRecipientQuantity FROM mtvdedicateapp_posts WHERE WSDedicationSuccess = 1 AND WSDedicationPrivate = 0 ORDER BY WSRecipientQuantity DESC LIMIT ".$offset.", ".$results;
			} else {
				$query = "SELECT *, COUNT(*) AS WSRecipientQuantity FROM mtvdedicateapp_posts WHERE WSDedicationSuccess = 1 AND WSDedicationPrivate = 0 ORDER BY WSRecipientQuantity DESC LIMIT ".$results;
			}
...
$result = $radio->most_received($results_per_page, $page);
